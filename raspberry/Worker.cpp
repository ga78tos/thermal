...
//freq_str = freq;
 // struct timespec start, end;
  ...
  system(("/opt/vc/bin/vcmailbox 0x00038002 12 8 3 " + freq_str + "000 1 > /dev/null").c_str());
...
//char buff[10];
//std::strcpy(buff, freq_str.c_str());
//int fild = open(freq_cpu[core], O_WRONLY | O_TRUNC);
//int res = write(fild, buff, std::strlen(buff));
//close(fild);
/*ofstream fs (freq_cpu[core], ios::out | ios::trunc);
if(fs.is_open()){
	fs << buff;
	fs.close();
}*/
//clock_gettime(CLCK, &end);
  //cout<< "####setfreq time:\t\t\t\t" << (TimeUtil::convert_us(end) - TimeUtil::convert_us(start)) << endl;
	...
	while(Pipeline::isSimulating())
	{
		...
		{	
			...
			do //ton loop
			{
				...
				//cout << "TotalExed = " << total_exed_ms << "++++" << times_pr[idx] <<endl;
				...
				if (total_exed > ton)
					stop = true;
				sem_post(&ptm_sem);
				//cerr << "time\t" << Statistics::getRelativeTime_ms() /*<< "\tjobid\t" << current_job->getId() */<< "\tfreq\t" << freqs_pr[idx] << endl;

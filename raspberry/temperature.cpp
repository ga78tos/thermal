...
void TempWatcher::timedJob(unsigned index){
 ...
 // struct timespec start, end;
  ...
  //system("ps -eo pid,pcpu,rtprio,ni,pri,comm,user,psr | grep main >> outt.txt");
  //system("vcgencmd measure_clock arm");
  //clock_gettime(CLCK, &end);
  //cout << "###" << (TimeUtil::convert_us(end) - TimeUtil::convert_us(start)) << endl;
  sem_wait(&temp_sem);
  resTemp = get_cpu_temperature();
  sem_post(&temp_sem);
  tempTrace.push_back(resTemp);
  //clock_gettime(CLCK, &end);
  //cout << "temp: " << (TimeUtil::convert_us(end) - TimeUtil::convert_us(start)) << endl;
...
temp_res_type TempWatcher::get_cpu_temperature(){
    ...
    "/sys/class/hwmon/hwmon2/temp2_input"};*/
    const char* n[] = {	"/sys/class/thermal/thermal_zone0/temp",

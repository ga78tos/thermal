...
int Parser::parseFile(){
	...
	vector<unsigned long> available_frequencies = {768000,800000,900000,1100000,1200000,1300000,1400000,1500000,1600000,1700000};
	//new frequency setting (0.4 .. 1), also select frequency closest to freq[x] (instead of next-smallest as in original)
	vector<unsigned long> range_edges ={
		available_frequencies[0] + (available_frequencies[1] - available_frequencies[0]) / 2,
		available_frequencies[1] + (available_frequencies[2] - available_frequencies[1]) / 2,
		available_frequencies[2] + (available_frequencies[3] - available_frequencies[2]) / 2,
		available_frequencies[3] + (available_frequencies[4] - available_frequencies[3]) / 2,
		available_frequencies[4] + (available_frequencies[5] - available_frequencies[4]) / 2,
		available_frequencies[5] + (available_frequencies[6] - available_frequencies[5]) / 2,
		available_frequencies[6] + (available_frequencies[7] - available_frequencies[6]) / 2,
		available_frequencies[7] + (available_frequencies[8] - available_frequencies[7]) / 2,
		available_frequencies[8] + (available_frequencies[9] - available_frequencies[8]) / 2};

	for(unsigned short i = 0; i < freq.size(); i++)
	{
		//calc frequency matching freq[x]
		float cur_freq = max(0.4f, freq[i]/1000.f);
		unsigned long assoc_freq = available_frequencies[0] + (available_frequencies[available_frequencies.size()-1] - available_frequencies[0]) * (cur_freq - 0.4f) / 0.6f;
		//cout << assoc_freq << "#" << freq[i] << endl;
		//find closest available freq to assoc_freq
		for(unsigned short j = 0; j < range_edges.size(); j++)
		{
			if(assoc_freq < range_edges[j])
			{
				freq[i] = available_frequencies[j];
				break;
			}
			if(j == range_edges.size() - 1)
				freq[i] = available_frequencies[j+1];
		}
